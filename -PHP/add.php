...
foreach ($input_fieldNames as $input_header){
    echo $input_header . ' <br>';
}

// finding the first column the user has defined input for.
/*for ($i = 0; $i < $count; $i++) if (isset($userInputs[$i])) {
    $offset = $i;
    break;
}*/
...
$headerString = $input_headers[0];

// entering the rest
for ($i = 1; $i < $table_headers_length; $i++){
    //if (isset($userInputs[$i])) {
    $inputString .= ", '" . $input_data[$i] . "'";
    $headerString .= ', ' . $input_headers[$i];
}
...
// EXECUTING QUERY
($conn->query($sql) === TRUE) ? $msg = 'New record created successfully' : $msg = 'Error: ' . $conn->msg;
...
// must be done afterwards, to fetch the AI album_ID (though i know there are other ways to get the next Auto Increment values.)
if (isset($_genres)) {
    ...
    for ($j = 0; $j < $_genres_count; $j++) {
        ...
        echo ($conn->query($_sql) === TRUE) ? 'genre ' . $_genres[$j] . ' added successfully' : '<script> alert("Error, klarte ikke legge til sjanger: ' . $conn->msg . '") </script>';
        }
}
