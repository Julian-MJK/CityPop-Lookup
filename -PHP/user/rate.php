<?php
    ...
    if (isset($subject) && isset($subject_id) && isset($rating)) {
        ...
        if ($conn->query('SELECT user_id FROM userRating_' . $subject)->fetch_assoc()) {
            ...
            echo deleteComposite('userRating_' . $subject, $subject . '_id', 'user_id', $subject_id, $user_id) ? 'Deleted existing rating' : 'Something went wrong, ' . $conn->error;
        }
        ...
        if ($success) {
            echo 'Successfully rated ' . $subject . '_id ' . $subject_id . ' ' . $rating . '/5 for user ' . $_SESSION['username'];
    ...
    if (isset($error)) {
        ...
        passTo('../../' . $subject . '/?a=' . $subject_id, ['msg', 'msgBGColor'], [addslashes($error), 'var(--red)']);
